...
const fetchCachedData = async (
  ...
  try {
    const cachedData = await withTimeout(
      env.SPOTIFY_API_HANDLER_CACHE.get(cacheKey, { type: "json" }),
      timeoutMs,
    );
    return cachedData;
  } catch (error) {
...
const fetchRandomTrack = async (
  ...
  const latestKey = await withTimeout(
    env.SPOTIFY_API_HANDLER_CACHE.get("latest-key"),
    5000,
  );
  ...
  await env.SPOTIFY_API_HANDLER_CACHE.put(
    cacheKey,
    JSON.stringify(trackFormatted),
    {
